...
const Material = () => {
  ...
  const getSubjectMaterial = () => {
    ...
    axios
      .post(`${baseApiURL()}/material/getMaterial`, { subject: selected }, { headers })
      .then((response) => {
        if (response.data.success) {
    ...
    <div className="w-full mx-auto mt-10 flex justify-center items-start flex-col mb-10">
      ...
      <div className="mt-8 w-full flex justify-center items-center flex-col">
        <div className="flex justify-center items-center w-[40%]">
          ...
          <button onClick={getSubjectMaterial} className="bg-blue-500 text-white py-3 px-4 text-2xl rounded-sm">
            <HiOutlineSearch />
          </button>
        </div>
        <div className="mt-8 w-full">
          ...
          {material && material.length === 0 && selected && <p className="text-center">No Material For {selected}!</p>}
        </div>
      </div>
    </div>
  )
