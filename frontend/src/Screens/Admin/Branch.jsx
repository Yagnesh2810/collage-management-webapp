    ...
    <div className="w-full mx-auto mt-10 flex justify-center items-start flex-col mb-10">
      ...
      {selected === "add" && (
        <div className="flex flex-col justify-center items-center w-full mt-8">
          ...
          <button className="mt-6 bg-blue-500 px-6 py-3 text-white" onClick={addBranchHandler}>
            Add Branch
          </button>
        </div>
      )}
          ...
          <ul>
            {branch &&
              branch.map((item, index) => {
                return (
                  <li key={index} className="bg-blue-100 py-3 px-6 mb-3 flex justify-between items-center w-[70%]">
                    <div>{item.name}</div>
                    <button className="text-2xl hover:text-red-500" onClick={() => deleteBranchHandler(item._id)}>
                      <MdOutlineDelete />
                    </button>
                  </li>
                )
