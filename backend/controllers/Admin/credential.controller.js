...
const loginHandler = async (req, res) => {
  ...
  try {
    const user = await adminCredential.findOne({ loginid })
    if (!user) {
...
const updateHandler = async (req, res) => {
  try {
    const user = await adminCredential.findByIdAndUpdate(req.params.id, req.body)
    if (!user) {
      return res.status(400).json({
        success: false,
...
const deleteHandler = async (req, res) => {
  try {
    const user = await adminCredential.findByIdAndDelete(req.params.id)
    if (!user) {
      return res.status(400).json({
        success: false,
        message: "No Admin Exists!",
